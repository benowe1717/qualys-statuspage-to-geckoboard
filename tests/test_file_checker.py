...
class TestFileChecker:
    ...
    def test_file_on_startup_relative_path_with_period(self):
        path = '/home/benjamin/Documents/qualys'
        path += '/qualys-statuspage-to-geckoboard/tests/data'
    ...
    def test_file_on_startup_user_path(self):
        path = '/home/benjamin/Documents/qualys'
        path += '/qualys-statuspage-to-geckoboard/tests/data'
        file = '~/Documents/qualys/qualys-statuspage-to-geckoboard'
        file += '/tests/data/credentials.yaml'
        fc = FileChecker(file)
        assert fc.file == f'{path}/credentials.yaml'
    ...
    def test_is_yaml(self):
        ...
        assert 'statuspage' in creds.keys()
        statuspage_creds = creds['statuspage']
        assert 'apikey' in statuspage_creds.keys()
        assert 'host' in statuspage_creds.keys()
        assert 'pageid' in statuspage_creds.keys()
        assert statuspage_creds['apikey'] == 'some-api-key-goes-here'
        assert statuspage_creds['host'] == 'api.statuspage.io'
        assert statuspage_creds['pageid'] == 'ra5h7xd8knxz'
        self.tearDown()
